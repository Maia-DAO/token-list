...
async function main() {
    ...
    for (const src of feeInfoTokens) {
        ...
        for (const [chainId, peerEntry] of Object.entries(peersInfo)) {
            ...
            const dst = tokens.find(t => t.chainId === parseInt(chainId) && t.address.toLowerCase() === peerEntry.tokenAddress.toLowerCase() && t.extensions?.peersInfo?.[src.chainId]?.tokenAddress.toLowerCase() === src.address.toLowerCase());
