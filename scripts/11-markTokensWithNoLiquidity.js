...
class TokenLiquidityChecker {
  ...
  extractAllTokens(tokenList, inactiveTokenList) {
    ...
    if (tokenList.rootTokens) {
      allTokens.push(
        ...tokenList.rootTokens.map((token) => ({
          ...token,
          source: 'rootTokens',
        }))
      )
    ...
    if (inactiveTokenList.tokens) {
      allTokens.push(
        ...inactiveTokenList.tokens.map((token) => ({
          ...token,
          source: 'inactive',
        }))
      )
  ...
  async processAllTokensFromLists(options = {}) {
    ...
    // Extract all tokens
    const allTokens = this.extractAllTokens(tokenList, inactiveTokenList)

    // Filter tokens if needed
    let tokensToCheck = allTokens
    if (filterBridgeTokensOnly) {
      tokensToCheck = allTokens.filter(
        (token) => token.isOFT || token.isAcross || token.extensions?.oftInfo || token.extensions?.acrossInfo
      )
    }
  ...
  filterTokens(tokenResults, options = {}) {
    ...
    return tokenResults.filter((result) => {
      ...
      if (
        ignoreTokens.some(
          (t) =>
            this.getTokenAddress(t).toLowerCase() === this.getTokenAddress(token).toLowerCase() &&
      ...
      const failsLiquidityChecks =
        (requireAnyLiquidity && !result.hasAnyLiquidity) || result.totalLiquidity < minimumLiquidity
...
async function runLiquidityCheck() {
  ...
  console.log(
    'Unsupported chains:',
    chainInfo.unsupportedChains.map((id) => `${CHAIN_KEYS[id]} (${id})`)
  )
    ...
    if (tokensToFlag.length > 0) {
      for (const tokenToFlag of tokensToFlag) {
        ...
        if (sourceToFlag === 'rootTokens') {
          ...
          const found = inactiveTokens.find(
            (t) => checker.getTokenAddress(t) === addrToFlag && t.chainId === chainIdToFlag
          )
          if (found) {
            found.noLiquidityOnChain = true
          } else {
    ...
    if (tokensToDelete.length > 0) {
      for (const tokenToDelete of tokensToDelete) {
        ...
        if (sourceToDelete === 'rootTokens') {
          tokenList.rootTokens = (tokenList.rootTokens || []).filter((t) => checker.getTokenAddress(t) !== addrToDelete)
