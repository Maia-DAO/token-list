...
async function main() {
  try {
    ...
    const inputFiles = [
      "output/uni_extended.json",
      "output/compound.json",
      "output/set.json",
      "output/ba.json",
    ];
    ...
    const newInactiveOutput = {
      ...
      logoURI:
        "https://raw.githubusercontent.com/Maia-DAO/token-list-v2/main/logos/Hermes-color.svg",
    };

    let finalInactiveOutput = newInactiveOutput;

    try {
      const existingInactiveRaw = await fs.readFile(
        "inactive-token-list.json",
        "utf8"
      );
      ...
      if (
        ...
        finalInactiveOutput.timestamp = Math.floor(
          Date.now() / 1000
        ).toString();
      } else {
    ...
    await fs.writeFile(
      "inactive-token-list.json",
      JSON.stringify(finalInactiveOutput, null, 2)
    );
    console.log(
      `✅  inactive-token-list.json written with ${finalTokens.length} tokens`
    );
  } catch (err) {
